...
public class Peg extends GameObject {
    ...
    public Peg(GameScreen screen, int level, int gfxPack, float size) {
        super(screen, screen.assets.assetMap.get(screen.artPack).get(AssetType.peg).getKeyFrames()[0], size, size);
    ...
    public void update(float dt, Vector2 mousePosition) {
        ...
        image = screen.assets.assetMap.get(screen.artPack).get(AssetType.peg).getKeyFrames()[0];
    }
}
